...
#--------------------------user confirm the work is done or not----------------

@app.route('/user/confirm_work/<int:request_id>', methods=['POST'])
@login_required
def confirm_work(request_id):
    # ensure only logged-in user confirms
    ...
    # security: user can confirm only their own request
    conn.execute("""
    ...
    flash('Work confirmed successfully! You can now proceed to payment.', 'success')
    return redirect(url_for('user_dashboard'))
